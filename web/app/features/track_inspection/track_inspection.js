		...
		function formatTimeWindow(startValue, endValue) {
			const start = normalizeUnixSeconds(startValue);
			const end = normalizeUnixSeconds(endValue);
		...
		function getVisibleHeavyLayerCount() {
			return layerOrder.filter(layerKey => {
				const cfg = layerConfig[layerKey] || {};
				const style = layerStyles[layerKey] || {};
				if (!style.visible) return false;
				if (isOdLayer(layerKey) || cfg.kind === "stations" || cfg.lineOnly) return false;
				return cfg.kind === "gps" || cfg.kind === "signal" || cfg.hasLine;
			}).length;
		}

		function getPerLayerMarkerBudget(layerKey, rowCount) {
			const cfg = layerConfig[layerKey] || {};
			if (!rowCount || isOdLayer(layerKey) || cfg.kind === "stations") return Number.POSITIVE_INFINITY;
			const zoom = getCurrentRenderZoomBucket();
			const heavyLayerCount = Math.max(1, getVisibleHeavyLayerCount());
			const totalBudget = zoom >= 15 ? 2200 : zoom >= 13 ? 1200 : zoom >= 11 ? 680 : 360;
			let budget = Math.max(40, Math.floor(totalBudget / heavyLayerCount));
			if (cfg.kind === "signal") budget = Math.max(30, Math.floor(budget * 0.65));
			if (cfg.kind === "gps") budget = Math.max(50, budget);
			if (rowCount > 3000 && zoom < 15) budget = Math.min(budget, zoom >= 13 ? 360 : 180);
			return budget;
		}
		...
		function buildLayerEntryText(layerKey, row, index) {
			...
			if (layerKey === "low_speed_segments") {
				const state = normalizeStateValue(row.state);
				const label = state === "short_low_speed" ? "短途低速(<30min)" : "低速驻留";
				const start = normalizeUnixSeconds(getRowStartTimeValue(row));
				const end = normalizeUnixSeconds(getRowEndTimeValue(row));
				const fallbackDuration = start != null && end != null ? end - start : 0;
				const startText = start != null ? formatTime(start) : "";
				const endText = end != null ? formatTime(end) : "";
				const timeText = `${startText}-${endText} | 持续 ${formatDuration(row.duration_s ?? fallbackDuration)}`;
				const radiusText = formatMeters(row.radius_m);
				const support = row.support_point_count || "-";
				const total = row.total_point_count || "-";
				const speed = parseNumericValue(row.p75_kf_speed_mps);
				const speedText = speed == null ? "" : ` | p75速度 ${speed.toFixed(2)}m/s`;
				return `${label} | ${timeText}${radiusText ? ` | 半径 ${radiusText}` : ""} | 支持点 ${support}/${total}${speedText}`;
			}
			if (cfg.kind === "gps") {
		...
		function getPolylineGroupField(layerKey, cfg = {}) {
			const configured = String(cfg.lineGroupField || cfg.polylineGroupField || "").trim();
			if (configured) return configured;
			if (layerKey === "line_semantic" || layerKey === "semantic_fmm_input") return "window_id";
			return "";
		}

		function buildPolylineSegments(layerKey, data, groupField = "") {
			const segments = new Map();
			const fallbackKey = "__all__";
			(data || []).forEach((row, rowIndex) => {
				const coord = getRowCoordinate(row);
				if (!coord) return;
				const groupValue = groupField ? String(row?.[groupField] ?? "").trim() : "";
				const key = groupValue || fallbackKey;
				if (!segments.has(key)) segments.set(key, []);
				segments.get(key).push({
					row,
					rowIndex,
					order: parseNumericValue(row?.point_order) ?? parseNumericValue(row?.segment_point_order) ?? rowIndex,
					time: getRowTimeValue(row) ?? rowIndex,
					coord: toGcj(coord.lat, coord.lon),
				});
			});
			return [...segments.entries()].map(([key, rows]) => {
				rows.sort((a, b) => {
				...
				return { key, coords: rows.map(item => item.coord) };
			});
		}

		function renderPolylineSegments(segments, style, cfg, targetGroup, profile, renderStats) {
			const drawableSegments = (segments || []).filter(segment => Array.isArray(segment.coords) && segment.coords.length >= 2);
			const sourcePointCount = drawableSegments.reduce((total, segment) => total + segment.coords.length, 0);
			let segmentsToDraw = drawableSegments;
			const maxPoints = profile.maxPolylinePoints;
			if (Number.isFinite(maxPoints) && maxPoints > 0 && segmentsToDraw.length * 2 > maxPoints) {
				segmentsToDraw = sampleEvenly(segmentsToDraw, Math.max(1, Math.floor(maxPoints / 2)));
			}
			const drawSourcePointCount = segmentsToDraw.reduce((total, segment) => total + segment.coords.length, 0);
			const scale = Number.isFinite(maxPoints) && maxPoints > 0 && drawSourcePointCount > maxPoints
				? maxPoints / drawSourcePointCount
				: 1;
			let remainingArrows = profile.maxArrows || 0;
			let renderedPointCount = 0;
			segmentsToDraw.forEach((segment, segmentIndex) => {
				const pointBudget = scale < 1
					? Math.max(2, Math.floor(segment.coords.length * scale))
					: maxPoints;
				const coords = sampleCoordinates(segment.coords, pointBudget);
				renderedPointCount += coords.length;
				...
				const remainingSegments = Math.max(1, segmentsToDraw.length - segmentIndex);
				const arrowBudget = remainingArrows > 0 ? Math.max(1, Math.floor(remainingArrows / remainingSegments)) : 0;
				const addedArrows = addSegmentArrows(coords, style.color, style.opacity, targetGroup, arrowBudget);
				remainingArrows = Math.max(0, remainingArrows - addedArrows);
			});
			renderStats.polylinePoints = renderedPointCount;
			renderStats.reduced = renderedPointCount < sourcePointCount || segmentsToDraw.length < drawableSegments.length;
			return renderStats;
		}
		...
		function renderOneLayer(layerKey, data, targetGroup, options = {}) {
			...
			const markerBudget = getPerLayerMarkerBudget(layerKey, data?.length || 0);
			if (Number.isFinite(markerBudget)) {
				profile.maxBuckets = Math.min(profile.maxBuckets, markerBudget);
				profile.maxTooltipBuckets = Math.min(profile.maxTooltipBuckets, Math.max(0, Math.floor(markerBudget * 0.08)));
			}
			const renderStats = {
				layerKey,
			...
			if (cfg.kind === "gps") {
				const lineGroupField = getPolylineGroupField(layerKey, cfg);
				if (cfg.hasLine && lineGroupField) {
					renderPolylineSegments(buildPolylineSegments(layerKey, data, lineGroupField), style, cfg, targetGroup, profile, renderStats);
				} else {
					const coords = [];
					data.forEach(row => {
						const coord = getRowCoordinate(row);
					...
					renderStats.reduced = sampledCoords.length < coords.length;
				}
				const lineReduced = renderStats.reduced;
				...
				renderStats.reduced = lineReduced || gpsBuckets.length < data.length;
				return renderStats;
			}
			...
			if (cfg.kind === "signal") {
				...
				renderStats.reduced = lineReduced || signalBuckets.length < data.length;
				return renderStats;
			}
			...
			renderStats.reduced = lineReduced || pointBuckets.length < data.length;
			return renderStats;
		}
