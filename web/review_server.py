...
def _sanitize_source_name(raw_name: str) -> str:
	...
	if suffix not in {".csv", ".zip"}:
		raise ValueError("only .csv or .zip uploads are currently supported")
	stem = Path(name).stem
...
class ReviewRequestHandler(SimpleHTTPRequestHandler):
	...
	def _normalize_signal6_algorithm_profile_for_upload(self, value, *, default: str = "speed_sparsity_90") -> str:
		text = str(value or "").strip()
		return normalize_signal6_algorithm_profile(text or default)
	...
	def _resolve_upload_source_path(self, upload_root: Path, payload: dict) -> Path:
		...
		for child in sorted(upload_root.iterdir()) if upload_root.exists() else []:
			if child.name in {UPLOAD_STATUS_NAME, "payload.zip", "payload.zip.part", "intake_report.json"}:
				continue
			if child.is_file() and child.suffix.lower() in {".csv", ".zip"}:
	...
	def _publish_upload_preview(self, upload_root: Path, catalog_path: Path, payload: dict, actor: ActorIdentity) -> dict:
		...
		signal6_algorithm_profile = (
			self._normalize_signal6_algorithm_profile_for_upload(payload.get("signal6_algorithm_profile"))
			if upload_type in {"signal6", "signal_triplet"}
			else ""
		)
		...
		if upload_type == "signal_triplet":
			return {
				"status": "uploaded",
				"upload_type": upload_type,
				"asset_id": asset_id,
				"batch_name": batch_name,
				"published_batch_name": "",
				...
				"error": "",
				"error_summary": "",
				"note": "多源轨迹包已上传；正式处理会运行 90% 展示算法并生成 signal/gate/lbs/snap/od/reconstruction/gps 七层结果。",
				"preview_only": False,
			}
		...
		dropped_signal_preview_rows = int(adapter_report.get("dropped_outside_beijing_bbox_rows") or 0)
		...
		return {
			...
			"error_summary": "",
			"note": (
				"仅上传已生成原始预览，可直接打开；后续仍可触发正式处理。"
				+ (
					f" 原始预览已忽略 {dropped_signal_preview_rows} 行北京范围外信令点。"
					if dropped_signal_preview_rows
					else ""
				)
			),
			"preview_only": True,
	...
	def _process_upload(self, upload_root: Path, catalog_path: Path, payload: dict, actor: ActorIdentity) -> dict:
		...
		record["signal6_algorithm_profile"] = signal6_algorithm_profile
		...
		lock_acquired = False
		if upload_type in {"signal6", "signal_triplet"}:
			lock_acquired = SIGNAL6_PROCESSING_LOCK.acquire(blocking=False)
			if not lock_acquired:
				raise UserUploadAdapterError(
					"another signal6 processing job is already running; wait for it to finish before starting a new one"
				)
		try:
			if upload_type == "trajectory4":
				adapter_report = build_trajectory4_result(source_path, result_root, title=title, field_mapping=field_mapping)
				ui_mode = "trajectory_layers"
				review_reference_files = ["gps.csv"]
				signal6_pipeline_mode = ""
			elif upload_type == "signal_triplet":
				signal6_pipeline_options = signal6_pipeline_options_for_profile(signal6_algorithm_profile)
				signal6_pipeline_mode = "v311"
				adapter_report = build_signal_triplet_result(
					source_path,
					result_root,
					title=title,
					pipeline_options=signal6_pipeline_options,
				)
				ui_mode = "trajectory_layers"
				review_reference_files = list(adapter_report.get("review_reference_files") or [])
			else:
				signal6_pipeline_options = signal6_pipeline_options_for_profile(signal6_algorithm_profile)
				signal6_pipeline_mode = signal6_pipeline_mode_for_profile(
					self.signal6_pipeline_mode,
					signal6_algorithm_profile,
				)
				adapter_report = build_signal6_result(
					source_path,
					...
					title=title,
					field_mapping=field_mapping,
					pipeline_mode=signal6_pipeline_mode,
					pipeline_options=signal6_pipeline_options,
				)
				ui_mode = str(adapter_report.get("ui_mode") or ("chain2" if signal6_pipeline_mode == "v311" else "trajectory_layers"))
				review_reference_files = list(adapter_report.get("review_reference_files") or [])
				if not review_reference_files:
					review_reference_files = ["line.csv", "fmm.csv"] if ui_mode == "chain2" else ["signal.csv"]
		finally:
			if lock_acquired:
				SIGNAL6_PROCESSING_LOCK.release()
		...
		extra_metadata = {
			...
			"source_asset_id": asset_id,
			"signal6_algorithm_profile": signal6_algorithm_profile if upload_type in {"signal6", "signal_triplet"} else "",
			"signal6_pipeline_mode": signal6_pipeline_mode if upload_type in {"signal6", "signal_triplet"} else "",
			"fmm_algorithm": adapter_report.get("fmm_algorithm") if upload_type in {"signal6", "signal_triplet"} else None,
			"ui_config": ui_config,
		}
		publish_report = publish_batch(
			published_root=published_batch_root.parent,
	...
	def _handle_create_upload(self, actor: ActorIdentity) -> None:
		try:
			...
			signal6_algorithm_profile = (
				self._normalize_signal6_algorithm_profile_for_upload(payload.get("signal6_algorithm_profile"))
				if upload_type in {"auto", "signal6", "signal_triplet"}
				else ""
			)
	...
	def _handle_upload_blob(self, upload_id: str, actor: ActorIdentity) -> None:
		...
		for pattern in ("*.csv", "*.zip"):
			for child in upload_root.glob(pattern):
