		function applyManifestUiConfig(manifest = null) {
			...
			const hideReviewPanelRaw = batchUiConfig?.hide_review_panel ?? manifest?.hide_review_panel ?? preset.hideReviewPanel;
			currentUiConfig.hideReviewPanel = hideReviewPanelRaw === true;
			const annotationEnabledRaw = batchUiConfig?.annotation_enabled ?? manifest?.annotation_enabled ?? preset.annotationEnabled;
			currentUiConfig.annotationEnabled = annotationEnabledRaw !== false;
		...
		function getGpsComparisonPayload() {
			const manifestPayload = currentManifest?.gps_comparison;
			if (manifestPayload && typeof manifestPayload === "object") return manifestPayload;
			const batchPayload = currentBatchMeta?.gps_comparison;
			if (batchPayload && typeof batchPayload === "object") return batchPayload;
			return null;
		}

		function formatGpsComparisonPercent(value) {
			const numeric = Number(value);
			if (!Number.isFinite(numeric)) return "--";
			return `${numeric.toFixed(numeric >= 99.95 ? 0 : 1)}%`;
		}

		function getGpsTruthLayerKeysForComparison() {
			return Object.entries(layerConfig || {})
				.filter(([key, cfg]) => key === "gps" || cfg?.kind === "gps")
				.map(([key]) => key);
		}

		function isGpsTruthLayerVisibleForComparison() {
			const controlsRoot = document.getElementById("layer-controls");
			if (controlsRoot) {
				const directGpsInput = controlsRoot.querySelector('.layer-row[data-layer="gps"] input[data-opt="visible"]');
				if (directGpsInput) return directGpsInput.checked;
			}
			const gpsLayerKeys = getGpsTruthLayerKeysForComparison();
			if (!gpsLayerKeys.length) return true;
			if (controlsRoot) {
				const renderedInputs = Array.from(controlsRoot.querySelectorAll(".layer-row[data-layer]"))
					.filter(row => gpsLayerKeys.includes(row.dataset.layer || ""))
					.map(row => row.querySelector('input[data-opt="visible"]'))
					.filter(Boolean);
				if (renderedInputs.length) return renderedInputs.some(input => input.checked);
			}
			return gpsLayerKeys.some(key => layerStyles[key]?.visible !== false);
		}

		function renderGpsComparisonPanel(uid = currentUid) {
			const panel = document.getElementById("gps-compare-panel");
			const accuracyEl = document.getElementById("gps-compare-accuracy");
			const detailEl = document.getElementById("gps-compare-detail");
			if (!panel || !accuracyEl || !detailEl) return;
			const comparison = getGpsComparisonPayload();
			if (!comparison) {
				panel.classList.add("hidden");
				accuracyEl.textContent = "--";
				detailEl.textContent = "当前批次暂无 GPS 比对";
				return;
			}
			if (!isGpsTruthLayerVisibleForComparison()) {
				panel.classList.add("hidden");
				accuracyEl.textContent = "--";
				detailEl.textContent = "GPS 真值轨迹图层已关闭";
				return;
			}
			const byUid = comparison.by_uid && typeof comparison.by_uid === "object" ? comparison.by_uid : {};
			const uidComparison = uid ? byUid[uid] : null;
			const percent = uidComparison?.accuracy_percent ?? comparison.overall_accuracy_percent;
			const threshold = uidComparison?.threshold_meters ?? comparison.threshold_meters;
			const matchedFile = uidComparison?.matched_gps_file || "";
			const totalPoints = uidComparison?.total_points ?? comparison.total_points ?? 0;
			const gpsCoveredLength = uidComparison?.gps_covered_length_m ?? comparison.gps_covered_length_m;
			const gpsTotalLength = uidComparison?.gps_total_length_m ?? comparison.gps_total_length_m;
			const gpsCoverageThreshold = uidComparison?.gps_coverage_threshold_m ?? comparison.gps_coverage_threshold_meters ?? comparison.road_segment_distance_meters ?? threshold;
			const roadTruthSegments = uidComparison?.road_truth_segments ?? comparison.road_truth_segments;
			const roadMatchedSegments = uidComparison?.road_matched_segments ?? comparison.road_matched_segments;
			const roadDistance = comparison.road_segment_distance_meters;
			const roadHeading = comparison.road_segment_heading_degrees;
			const hasLengthCoverage = Number.isFinite(Number(gpsTotalLength)) && Number(gpsTotalLength) > 0;
			const hasRoadMetric = Number.isFinite(Number(roadTruthSegments)) && Number(roadTruthSegments) > 0;
			const label = String(comparison.comparison_label || "比对 gps").trim();
			const labelEl = panel.querySelector(".gps-compare-label");
			if (labelEl) labelEl.textContent = label;
			panel.classList.remove("hidden");
			accuracyEl.textContent = formatGpsComparisonPercent(percent);
			if (hasLengthCoverage) {
				const coveredKm = (Number(gpsCoveredLength) / 1000).toFixed(2);
				const totalKm = (Number(gpsTotalLength) / 1000).toFixed(2);
				const thresholdText = Number.isFinite(Number(gpsCoverageThreshold)) ? ` · 阈值 ${Math.round(Number(gpsCoverageThreshold))}m` : "";
				detailEl.textContent = uidComparison
					? `当前 UID · 覆盖 ${coveredKm} / ${totalKm} km${thresholdText}${matchedFile ? ` · ${matchedFile}` : ""}`
					: `全批次 · 覆盖 ${coveredKm} / ${totalKm} km${thresholdText}`;
				return;
			}
			if (hasRoadMetric) {
				const roadDetail = `${Math.round(Number(roadMatchedSegments) || 0)} / ${Math.round(Number(roadTruthSegments) || 0)} 路段`;
				const ruleDetail = `路段阈值 ${Math.round(Number(roadDistance) || 0)}m / ${Math.round(Number(roadHeading) || 0)}°`;
				detailEl.textContent = uidComparison
					? `当前 UID · ${roadDetail} · ${ruleDetail}${matchedFile ? ` · ${matchedFile}` : ""}`
					: `全批次 · ${roadDetail} · ${ruleDetail}`;
				return;
			}
			detailEl.textContent = uidComparison
				? `当前 UID · ${totalPoints} 点 · 阈值 ${Math.round(Number(threshold) || 0)}m${matchedFile ? ` · ${matchedFile}` : ""}`
				: `全批次 · ${totalPoints} 点 · 阈值 ${Math.round(Number(threshold) || 0)}m`;
		}
