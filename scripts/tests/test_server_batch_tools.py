...
class ServerBatchToolsTest(unittest.TestCase):
	...
	def _write_heterogeneous_result_tree(self, root: Path) -> None:
		result_root = root / "result"
		result_root.mkdir(parents=True, exist_ok=True)
		(result_root / "manifest.json").write_text(
			(
				'{"uids":["9001"],'
				'"layers":["gps","tier4"],'
				'"layer_specs":{"gps":{"filename":"gps.csv","kind":"gps"},'
				'"tier4":{"filename":"tier4.csv","kind":"signal","review_reference":true}},'
				'"review_reference_files":["tier4.csv"]}\n'
			),
			encoding="utf-8",
		)
		(result_root / "states_index.json").write_text('{"9001":[]}\n', encoding="utf-8")
		uid_dir = result_root / "9001"
		uid_dir.mkdir(parents=True, exist_ok=True)
		(uid_dir / "gps.csv").write_text("uid,latitude,longitude,timestamp_ms\n9001,39.9,116.3,1\n", encoding="utf-8")
		(uid_dir / "tier4.csv").write_text("uid,cid,lat,lon,t_in,t_out\n9001,10,39.9,116.3,1,2\n", encoding="utf-8")
	...
	def test_validate_result_root_supports_manifest_declared_layer_files(self) -> None:
		dataset_root = self.root / "datasets" / "heterogeneous"
		self._write_heterogeneous_result_tree(dataset_root)
		report = validate_result_root(dataset_root / "result", require_review_reference=True)
		self.assertTrue(report["ok"])
		self.assertEqual(report["summary"]["layer_files"], ["gps.csv", "tier4.csv"])
		self.assertEqual(report["summary"]["review_reference_files"], ["tier4.csv"])
	...
	def test_intake_upload_bundle_accepts_manifest_declared_review_reference(self) -> None:
		upload_root = self.root / "incoming" / "20260416T000000Z_layered"
		upload_root.mkdir(parents=True, exist_ok=True)
		(upload_root / "upload_status.json").write_text('{"upload_id":"u2","status":"uploaded"}\n', encoding="utf-8")
		payload_path = upload_root / "payload.zip"
		with zipfile.ZipFile(payload_path, "w") as archive:
			archive.writestr(
			...
			archive.writestr("result/states_index.json", '{"9001":[]}\n')
			archive.writestr("result/9001/gps.csv", "uid,latitude,longitude,timestamp_ms\n9001,39.9,116.3,1\n")
			archive.writestr("result/9001/tier4.csv", "uid,cid,lat,lon,t_in,t_out\n9001,10,39.9,116.3,1,2\n")

		report = intake_upload_bundle(
			upload_root=upload_root,
			workspace_root=self.root / "workspaces" / "layered",
			published_root=self.root / "shared" / "published",
			batch_name="layered_b01",
			clean_workspace=True,
			require_review_reference=True,
			publish=True,
		)
		...
		self.assertTrue(report["validation"]["ok"])
		self.assertEqual(report["validation"]["summary"]["review_reference_files"], ["tier4.csv"])
